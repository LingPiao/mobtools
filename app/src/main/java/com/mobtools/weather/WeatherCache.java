...
public class WeatherCache {
    ...
    private void cacheInFile(String cityName, CityWeather cw) {
        ...
        LogUtil.d("Caching weather info in file[" + filename + "]...");
        ...
        try {
            ...
            LogUtil.w("Caching weather info in file[" + filename + "] exception,message:" + ex.getMessage());
        }
    }
    ...
    private void rebuildCacheFromFiles() {
        ...
        for (int i = 0; i < cities.length; i++) {
            ...
            LogUtil.d("Rebuilding weather info from file[" + filename + "]...");
            try {
                ...
                LogUtil.w("Rebuilding weather info from file[" + filename + "] exception,message:" + ex.getMessage());
            }
        }
    }
    ...
    public boolean isUpdateRequired() {
        ...
        LogUtil.d("WeatherCache is not required to update, now-lastLoaded=" + diff);
        return false;
    }
    ...
    public void clearCache() {
        ...
        for (int i = 0; i < cities.length; i++) {
            String filename = dir.getPath() + "/" + cities[i];
            LogUtil.d("Cleaning cached file[" + filename + "]...");
            try {
                File f = new File(filename);
                f.delete();
            } catch (Exception ex) {
                LogUtil.w("Cleaning cached file[" + filename + "] exception,message:" + ex.getMessage());
            }
        }
        cache.clear();
    }
