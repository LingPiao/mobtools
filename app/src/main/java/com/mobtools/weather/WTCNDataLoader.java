...
public class WTCNDataLoader extends AsyncTask<String, String, String> {
    ...
    private static List<String> days = new java.util.ArrayList<String>();
    ...
    private boolean loadWeatherInfo(String cityName) {
        ...
        try {
            ...
            if (cd == null) {
                Document cityDoc = Jsoup.connect(CITY_URL.replace(CITY_HOLDER, cityName) + System.currentTimeMillis()).ignoreContentType(true).referrer(DEFAULT_REFFER).get();
                String s = cityDoc.text(); //([{"ref":"10102010003A~
                if (s == null || s.length() < 6) {
                    info = "Can not find the city code for " + cityName;
                    LogUtil.w("==============Get weather info error:" + info);
                    errorMsg = info;
                    return false;
                }
            ...
            LogUtil.e("Get weather info error:", e);
            info = "Get weather info error:" + e.getMessage();
        }

        if (loaded) {
            // LogUtil.d("==============loadedWeatherInfo:" + info);
    ...
    protected String doInBackground(String... arg0) {
        // LogUtil.d("==============WTCNDataLoader.doInBackground");
        boolean loaded = false;
        ...
        if (loaded) {
            ...
            if (errorMsg.length() > 1) {
                LogUtil.d("============== doInBackground: showing error message");
